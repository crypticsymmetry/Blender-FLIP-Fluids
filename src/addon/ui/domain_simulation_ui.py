...
def draw_resolution_settings(self, context, master_column):
    ...
    if not sprops.simulation_method_expanded:
        ...
        if aprops.enable_adaptive_phase_field_level_set:
            ...
            column.prop(aprops, "adaptive_phase_field_far_distance")
            row = column.row(align=True)
            row.prop(aprops, "adaptive_phase_field_smoothing_iterations")
            row.prop(aprops, "adaptive_phase_field_smoothing_band_layers")
            column.prop(aprops, "adaptive_phase_field_smoothing_time_step")
            row = column.row(align=True)
